...
namespace Telescope.MSALClient;
...
public class PublicClientSingleton
{
    ...
    private PublicClientSingleton()
    {
        ...
        using var stream = assembly.GetManifestResourceStream("Telescope.appsettings.json");
        IConfiguration appConfiguration = new ConfigurationBuilder()
            .AddJsonStream(stream)
            .Build();

        var azureADConfig = appConfiguration.GetSection("AzureAd").Get<AzureAdConfig>();
        this.MSALClientHelper = new MSALClientHelper(azureADConfig);

        var downStreamApiConfig =
            appConfiguration.GetSection("DownstreamApi").Get<DownStreamApiConfig>();
